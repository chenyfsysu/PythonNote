...
class SkillManager(object):
    ...
    # - 多段连击技
    # - 同一槽位技能使用后后继技能的切换
    # 实现技能衔接约束/被打断/兼容三段击效果
    # - skillRunning
    # - COMBOSTART接收技能使用信号，COMBOEND接收技能结束信号
    # - 在START前和END后，技能起手动作的使用都是用JUMPTOSTATE进行切换
    # - 在START和END之间，则使用FIREEVENT来进行切换
    # - 同时用来约束主角使用技能时，技能动作未触发COMBOEND不能使用下一技能
    class Slot(object):
        def __init__(self, skillid, slotid, ownerid):
            ...
            self.skillid_cache = -1  # 默认值为-1，为默认值时将会重算skillid
    ...
    class ComboSlot(Slot):
        ...
        def skillid(self):
            ...
            skillid = default_skill  # 默认技能编号

            if self.cur_skillid:  # slot正在使用中
    ...
    def _get_ordinary_skills(self):  # noqa
        skills = []
        owner = EntityManager.getentity(self.ownerid)
    ...
    def getAvlSkills(self):  # noqa
        owner = EntityManager.getentity(self.ownerid)
        # 0 普攻, 1-4 and 5-8 八个正常技能 9 门派轻功 10, 11 装备特技 12, 13 副本道具技能 14 孩子主动技能 15 觉醒主动技能
...
class AvatarMember(cCombatUnit):
    ...
    def getSpeedUpperLimit(self):
        return const.PLAYER_SPEED_UPPER_LIMIT

    def getVisiability(self):
        if GlobalData.hideAllAvatar:    # 隐藏所有的avatar时，把伙伴也隐藏
            return False

        player = GlobalData.p
        if self.hsRole == HsConst.ROLE_HIDE:  # 捉迷藏：藏匿者始终可见，不被屏蔽按钮屏蔽
            return True
        ...
        if getattr(GlobalData, 'F11_flag', 0) and (not isPhotoing):
            if player.relation(self) == const.RELATION_IGNORE:
                return False
    ...
    def getModelData(self):
        ...
        if self.isFaking:
            fakeData = self.fakeData
            model_data = DataHelper.GetSchoolModelData(
                fakeData.school, fakeData.equips, model_data, fakeData.dressing,
                body=fakeData.body, dresscolor=fakeData.dressingColor,
            )
        else:
            ...
            model_data = DataHelper.GetSchoolModelData(
                self.school, equips, model_data,
                self.tryDressing if getattr(self, 'isTryingDress', False) else self.dressing,
                body=self.body,
                dresscolor=self.tryDressingColor if getattr(self, 'isTryingDress', False) else dressingColor,
                cloakModelChoose=cloakModelChoose,
            )
    ...
    def getSurvivalModelData(self):
        ...
        model_data = DataHelper.GetSchoolModelData(self.survivalSchool, equips, model_data,
                                                   None, dresscolor=dresscolor)
    ...
    def getSurvivalUIModelData(self, protoNo):
        ...
        equipmentsGetter = self.survivalGameItems.equipments.get
        equips = [x for x in [equipmentsGetter(part, dict()).get('id', 0) for part in
                              const.EQU_BODYPARTS] if x]
        dressingColor = DressColor.copy()
        dresscolor = getattr(self, 'view_dressing_color', dressingColor)
    ...
    def isProtectedFromAvts(self):
        if self.pkRole == const.ROLE_VIOLENT:
            return False
    ...
    def onDead(self, killer=0, skillid=0, skilllevel=0):
        if self.isQinggong():
            self.stopQinggong()
        cCombatUnit.onDead.im_func(self, killer, skillid, skilllevel)  # 复活动作等
    ...
    def onRelive(self):
        cCombatUnit.onRelive.im_func(self)  # 复活动作等
    ...
    def onBeDamaged(self, skillid, skilllevel, damage, attacker, atktype, atkindex=0):  # noqa
    ...
    def attractedToPos(self, pos, time):
        ...
        _motion = self.model.applyMotion(False)  # check
        _gravity = self.model.applyGravity(False)  # check
        _obstacle = self.model.navigator.EnableObstacle  # check
        ...
        # TODO:
        # 吸引过程中如果被其它的事件将navigator.cancel()，用时间保证回调仍然可以生效
    ...
    def triggerEnterStory(self, graph, endEvent, showSelf, showOthers, canSkip, interactive=0, spaceEnd=0):
        if interactive:
            ...
            if doll is None or model is None:
                utils.CallUntil(
                    func=lambda: self.doTriggerEnterStory(eid, graph, endEvent, showSelf, showOthers, canSkip, interactive),
                    condition=lambda: self.space.getEntityByID(eid) is not None and getattr(self.space.getEntityByID(eid), 'model', None),
                    delay=1.0
                ).start()
            else:
                doll.model.enterStory(graph, endEvent=endEvent, showSelf=showSelf, showOthers=showOthers, canSkip=canSkip, spaceEnd=spaceEnd)
        else:
            if getattr(GlobalData.p, 'model', None):
...
class PlayerAvatarMember(AvatarMember):
    ...
    Property("skillSwitch", 0)  # 技能键位页面选择
    ...
    def _on_set_invisible(self, old):
        ...
        self.attack_show_model = const.SKILL_WAIT_UNKNOWN
    ...
    def _checkTargetForSkill(self, target, skill):
        if skill.getType() != const.SKILL_TYPE_HEAL and target and target.IsAvatar:
    ...
    def calcAttackResult(self, targets, skillid, skilllevel, misc):  # TODO  # noqa
        ...
        if self.server_first:
            ...
            if skill:
                for target in targets:
                    ...
                    if skillType != const.SKILL_TYPE_BUFF and len(targets) >= skill.getMinAffectNum():
                        if skillType != const.SKILL_TYPE_HEAL:
                            atktype = const.ATTCK_T_NORMAL
                        else:
                            atktype = const.ATTCK_T_HEAL
                        damage = [[0, 0], [0, 0], [0, 0]]
                        bld = 0
    ...
    def switchSpecialSkill(self, skillid, com=None):
        ...
        if skill.level == 0:
            PopmsgPool().AddMsg("人物等级尚未达到%s解锁等级" % skill.name)
            return
        if skill.enable == 0:
            PopmsgPool().AddMsg("%s尚未习得，无法激活使用" % skill.name)
            if com:
    ...
    def getCanLvUpSkills(self):  # noqa
        if self.getSkillSourceid() != const.SKILL_SRC_ORIGIN:
            return {}
    ...
    def getSkillLvUpLmt(self, skill_idx):
        ...
        if skill_idx not in basic_skills:
            for idx in basic_skills:
                ...
                if skill_type == const.SCHOOL_SKILL_TYPE_PASSIVE:
    ...
    def _transToSkillAction(self, skillid, targetinfo, choose_pos, location_info):
        ...
        if self.SkillDisable(skillid):
            return False
        if skill_proto:
            if sstate <= const.SKILL_STATE_PROGRESS and sskill is not None and sskill.isComboSucceed(skillid):
                res = self.model.FireEvent(target_gid, const.USE_SKILL % skill_proto)
                self.setEndActionArgs((skillid, targetinfo, choose_pos, location_info))
                return res
            # 技能打断事件处理，镜头归位，屏幕效果归位
            if hasattr(self, 'interruptSkill'):
                self.interruptSkill(skillid)
            res = self.model.JumpToState(target_gid, "skill_%d" % skill_proto, 0.00)
        ...
        if res:
            ...
            # self.logger.info('skill_pos A -> %s %s', skill_pos, location_info)
    ...
    def waitAutoFight(self):
        ...
        delay = space_data.data.get(self.spaceno, {}).get('AutoCombatDelayLeader', 0) if self.isTeamLeader() else \
            space_data.data.get(self.spaceno, {}).get('AutoCombatDelay', 0)
        if not delay:
            return
    ...
    def startAutoFight(self):
        ...
        if now - last > delay:
            if self.isTeamLeader():
                MessageBox().show('确认进入自动挂机状态？', {
                    'callback': lambda: setattr(self, 'autoFightTimer', self.add_timer(delay, self.startAutoFight)),
                }, {
                    'callback': self.triggerAutoFight,
                    'countdown': 5,
                })
            else:
    ...
    def stopAutoFight(self):
        ...
        if getattr(self, "showTaskDebugInfo", False):
            if self._curBtreeid:
                msg = "停止%s" % ('任务AI' if self._curBtreeid == 'bttask' else '行为树AI')
    ...
    def switchTaskAutoFight(self):
        ...
        if self._curBtreeid != "bttask":
            if self.isTeamMember() and self.acceptTeamControl:
                self.changeTeamStatus(const.TEAM_STATUS_NORMAL)
                self.stopAutoFight()
                self.popNotificationMsg("你已取消跟随队长状态")
        self.AIRun('bttask')
        PlayerMain().updateAutoFight()
    ...
    def obtainMonsterKilledAward(self, monster):
        ...
        graph = HRD.data.get(25, {}).get('path', '') if self.school == const.SCHOOL_ID_TJ else HRD.data.get(24, {}).get('path', '')

        self._collectGraphId = self.model.PushGraph(graph, 0.1, 0)  # check
    ...
    def onBeDamaged(self, skillid, skilllevel, damage, attacker, atktype, atkindex=0):  # noqa
        ...
        if atktype not in (const.ATTCK_T_HEAL, const.ATTCK_T_RELIVE) and not self.target and not shotSkillTarget:
        ...
        if getattr(self, 'delay_move_info', None):
            ...
            if flag == const.SYNC_POS_FLAG_PASSIVE:
                self.position = (x, y, z)
    ...
    def calcSkillPosition(self, *args, **kwargs):  # TODO  # noqa
        if getattr(self, 'delay_move_info', None):
            x, y, z, flag = self.delay_move_info
            if flag == const.SYNC_POS_FLAG_INITIATIVE:
                self.delay_move_info = None
                return x, y, z
    ...
    def lockTarget(self, target):  # noqa
        if target and target.IsMonster and not target.isAlive():  # 怪物死亡后不能被选为目标，防止自动战斗时，鞭尸的情况
            return

        if getattr(target, 'shield_flag', 0) >= 2:
            return

        if not self.detective and (getattr(target, 'invisible', 0) and self.relation(target) != const.RELATION_COMRADE):  # 无反隐状态无法锁定隐身目标
            return
        ...
        # 如果是非攻击目标,如法术墙（远离法术墙时),直接返回，因为他们没有血法信息。。。。
    ...
    def showAutoPosChooser(self, skill):
        if skill.showAutoPosChooser():
            ...
            content = {
                ...
                "ownerid": self.id,
                "speed": float(speed),
                "position": formula.horizontalCoordinate(self.position, self.yaw, float(dist), 0),
                "direction": self.yaw,
            }
    ...
    def setChargeDirection(self, cancel=False, reset=False):
        ...
        if getattr(self, 'charge_effectid', None) is None:
            self.charge_effectid = self.model.attachEffect(effectid, 'Scene Root')  # check
    ...
    def OnStartCharing(self, slotid):
        ...
        if skill.isChargeSkill() and self.stateIM.applyInstruction(SICD.CHARGE):
            if self.invisible:
                ...
                self.server.removeAllInvisibleBuffs()
                return const.SKILL_WAIT_INVISIBLE
            ...
            self.server.onChargeSkill(skillid, chargeEndTime)  # to server
            self.onChargeStart(skillid)  # client action
            ...
            return const.SKILL_OK
        return const.SKILL_FAILED
    ...
    def OnChargeFinish(self, slotid, stamp=None):
        ...
        if charge_min_time_cd:
            ...
            if charge_min_time > 0 and chargeTime < charge_min_time:
                PlayerMain()._showSkillMessage(const.SKILL_CHARGE_TIME)
    ...
    def interruptSkill(self, skillid):
        # 技能打断的逻辑，reset相机，屏幕效果之类
        curSkillid, curSkillState = self.skillstate.skillRunning
        if curSkillState > const.SKILL_STATE_POSTCAST:
            # 后摇点之后，不强制打断镜头和屏幕效果
            return
        if self.id != GlobalData.p.id:
            return
    ...
    def OnJoyStickSelectBegin(self, slotid):
        ...
        effect2 = effects[1:2]
        content = {
            "effectid": effect1[0][0] if effect1 else '',
            "effectradius": effect1[0][1] if effect1 else 1.0,
            "ownerid": self.id,
            "speed": skill.JoyStickSelEffectSpeed(),
            "warn_effectid": effect2[0][0] if effect2 else '',
            "warn_effectradius": effect2[0][1] if effect2 else 1.0,
        }
    ...
    def OnSlotEnd(self, slotid):
        ...
        if getattr(self, 'comboholding', None):
            ...
            # 不能强制跳到locomotion，因为伤害结算、特效播放
            # 的流程都已经进行了。解决的办法是图里面的combostart
            # 点尽量靠后
    ...
    def OnSlotUsed(self, slotid):  # noqa
        ...
        if skill.chooseMode:
            ...
            if getattr(self, 'chooseInnerEffect', None):
                effect = self.model.getCueTypeEffect(self.chooseInnerEffect)
            if not effect:
                return const.SKILL_CHOOSE_MODE_ERR
            effect = effect[0]
            if skill.chooseMode == ChooseMode.Direction:
    ...
    def shouldUseTeamTarget(self, skill, targets):
        if not skill:
            return False
        skillType = skill.getType()
        return skillType == const.SKILL_TYPE_HEAL and not targets and self.isInTeam() and self.teamTarget
    ...
    def SpecialSkillCheck(self, skill):
        if skill.getSpecialEffectType() == const.ATK_EFFECT_CHASE:
    ...
    def testObstacle(self):
        ...
        if not self.model.isValid():
            return
        if self.inOfflineSpace():  # 离线本不检测阻挡 by剑林
            return True
        ...
        transform = self.model.model.Transform  # check
        pos1 = transform.translation
        pos1.y += 1.5
        pos2 = self.target.model.model.Transform.translation  # check
        pos2.y += 1.5
        ...
        if self.invisible:
            ...
            for buff in self.buffs.itervalues():
                ...
                if skillid not in buff.getKeepInvisibleSkills():
                    skill_avl = False
                    break
            # 如果不在白名单内，则先换回正常显示再使用技能
            if not skill_avl:
                if not getattr(self, 'attack_show_model', 0):
                    self.attack_show_model = const.SKILL_WAIT_MODEL
                    self.wait_show_model_info = (skillid, seltgts, choose_pos, restricttgts)
                    self.updateVisibleEntity(self.id)
                    return const.SKILL_WAIT_INVISIBLE
                elif self.attack_show_model == const.SKILL_WAIT_MODEL:
                    self.wait_show_model_info = (skillid, seltgts, choose_pos, restricttgts)
                    return const.SKILL_WAIT_INVISIBLE
                elif self.attack_show_model == const.SKILL_WAIT_READY:
                    self.attack_show_model = const.SKILL_WAIT_OK
                    self.wait_show_model_info = None
        return const.SKILL_OK

    def actionOnWaitModelReady(self):
        if getattr(self, 'attack_show_model', 0) == const.SKILL_WAIT_MODEL:
            self.attack_show_model = const.SKILL_WAIT_READY
    ...
    def doAttack(self, skillid, seltgts=None, choose_pos=None, restricttgts=None):  # noqa

        if self.isInSurvivalBattle() and self.model_shapeshift:
            self.popNotificationMsg('变身状态下无法使用技能')
            return
        if self.isInSurvivalBattle() and not self.isSurvivalSkillsValid(skillid) and skillid not in SurvivalBattleConst.ITEM_SKILL_LIST:
            self.popNotificationMsg('请装备有效武器')
            return
        if self.isInSurvivalBattle() and not self.canUsingSurvivalSkill(skillid) and skillid not in SurvivalBattleConst.ITEM_SKILL_LIST:
            self.popNotificationMsg('上一把武器仍有技能CD中，需要等待%d秒才能再次释放技能' % self.maxSkillCD)
            return
        ...
        self.clientInput()  # 客户端有输入，用来判定是否挂机

        skill = self.GetSkill(skillid)
        if skill is None:
            return const.SKILL_NOT_FOUND

        self.updateShowSkillRange(skill)

        if self.shouldUseTeamTarget(skill, seltgts):  # 将锁定目标自动切换为队伍UI假锁定的队友, 方便加血技能操作
        ...
        if t == const.SKILL_SUBSTITUTE:
            # 技能发生了替换
            return self.doAttack(self.substituteSkillid, seltgts, choose_pos)
        elif t == const.SKILL_DISTANCE:  # 超出技能射程,靠近目标
            if not self.target:
                return const.SKILL_NEEDTGT
            ...
            if general_attack_skill == skillid and sprint_distance and utils.getNow() - self.lastSkillSprt >= sprint_cd:
                ...
                # 不是普攻技能 或者 不要求普攻冲刺辅助 移动靠近
                srange = skill.getRange()
                arange = skill.getAssistantRange() if (skill.getAssistantType() in (const.STA_SPRINT_FACE,) and len(skill.getAssistantGraph())) else 0
        ...
        if self.isInSurvivalBattle() and t == const.SKILL_OK:
    ...
    def onAttackResultFix(self, result):
        # print 'onAttackResultFix >>> >>> >>>', result
        result['fixed'] = True
        cCombatUnit.onAttackResult.im_func(self, result)
    ...
    def walk(self, dest, arrived=None, radius=0.0, showMsg=True, forbid3D=False):  # noqa
        ...
        if not self.space.isInWalkableArea(dest, radius=obstacleRadius):
            self.popNotificationMsg("目标地点不可自动寻路")
            print "PlayerAvatar not walkable", dest
            return

        if not self.model.canMoveTo(dest):
            self.popNotificationMsg("寻路失败，请尝试移动到主干道")
    ...
    def getWalkEntity(self):
        if getattr(self, 'd_ride_info', None) and self.d_ride_info[1] == const.DR_SIDE_ATTACH and self.d_ride_info[0] == getattr(self, 'combatPokemonId', None):
    ...
    def checkCommonCD(self, skill):
        ...
        if not lastSkill or self.skills[lastSkill].comboSucc == skill.id:  # 三段击间迁移，无视公共cd
            return True
        else:
            if time.time() < self.GetSkillCCD()[1]:
                return False
        return True
    ...
    def fixAnticipatedSkill(self, result, newRandomState):
        ...
        # print 'fixAnticipatedSkill >>> >>> >>> >>> >>> >>> >>>', result
    ...
    def doAttackResultCalc(self, targets, skillid, skilllevel, misc):
        ...
        # 记录上次打过的目标
        skillType = skill.getType()
        if skillType != const.SKILL_TYPE_HEAL and hasattr(self, '_targetid') and self._targetid in targetids:
            target = self.target
            if target and decide(self, target) == const.RELATION_ENEMY:
    ...
    def SkillMisc(self, skill, targets):
        ...
        cdDecrease = self.buffs.sumAttr('cd_decrease') + self.getPskillsSumbAttr('cd_decrease') + self.getChildSkillCDRatio(skill.id)  # noqa
        if cdDecrease > 0:
    ...
    def isSkillReady(self, skill):
        ...
        if not self.stateIM.checkInstruction(SICD.USESKILL):
            ...
            res = const.SKILL_SKILLMGR
        elif skill.level == 0:
            res = const.SKILL_FAILED
        elif not skill.enable:
            res = const.SKILL_NOT_ENABLE
    ...
    def canUseSkill(self, combatunit, skill):
        if self.isSkillReady(skill) != const.SKILL_OK:
            res = False
    ...
    def setPanelSkills(self, panelSkills):
        if self.getCurrentPanelSkills() != panelSkills:
            seconds = panelSkills.getSkillIds()[const.MAX_PANEL_SKILLS:]
    ...
    def setSkillsSwitch(self, select=0):
        ...
        if not panelSkills.getSkillIds():
            # PopmsgPool().AddMsg("请先在技能配置页面添加技能")
            return
    ...
    def getCurrentPanelSkillPage(self):
        ...
        for i in xrange(len(panelSkills)):
            ...
            if skills.count(0) != len(skills):
                res += 1
        return min(res, const.MAX_PANEL_SKILLS)
    ...
    def onEnterCombat(self):
        ...
        self.checkIsInHomesteadArea()  # 是否在家园中的延迟检测
    ...
    def tryLeaveCombat(self):
        ...
        self.add_timer(const.COMBAT_OVER_TIME_FAST, functools.partial(self._doTryLeaveCombat, self.leaveCombatStamp))
    ...
    def CheckTgtlockFirst(self, tgtlock, skill):
        if skill.getType() == const.SKILL_TYPE_HEAL and getattr(tgtlock, 'id', 0) in self.teammember:
            return True
